...
| `--brand-secondary` | `oklch(0.52 0.13 45.3)` (`#A44C1D`) | La terracota como **primer plano**: etiquetas de marca favorita, `BrandStoreLink` y glifo de categoría. Es `Terracotta/text` del hub: 5.4–5.6:1. Partida de `--secondary` el 28-ago-2026, igual que `--brand` del verde. |
...
**Estado: veintiún marcos en tres anchos.** Siete pantallas de escritorio a 1440 —landing, acceso, agenda, seres queridos, ficha de persona, ideas de regalo y ajustes—, cuatro de tableta a 1024, tres de móvil a 390 más la hoja de navegación como overlay, y seis gemelos en oscuro: agenda, ajustes, la hoja, ideas de regalo, seres queridos y la ficha de persona. El recorrido de escritorio pasa por las siete y vuelve a la agenda; el de móvil repite el mismo camino y añade la hoja de navegación.
...
En el código son tres intercambios de clase, ninguno estructural: `app/(app)/layout.tsx:29` `hidden lg:flex` → `hidden xl:flex`, `layout.tsx:61` `lg:hidden` → `xl:hidden`, y `components/gifts/GiftsPanel.tsx:633` `lg:hidden` → `xl:hidden` para la barra fija que aparece al hacer scroll. **Mientras no se hagan, gana el código y esos cuatro marcos son propuesta**, dicho en una nota ámbar sobre la propia sección y en la cabecera de la página.
...
**Formulario de Community, listo para pegar.** Título: *PickPal Design System*. Categoría: *Design systems*. Licencia: *CC BY 4.0*. Etiquetas: `design system`, `design tokens`, `light and dark modes`, `wcag 2.2 aa`, `figma variables`. Descripción (inglés):

> PickPal is a Spanish web app for remembering the birthdays and anniversaries of the people you care about, and for finding a gift when one of those dates is close. This is its design system, built alongside the product and documented for people who have never seen it.
>
> Inside: 374 variables in three collections with Light and Dark as real modes; a ten-token brand hub that is the only place the identity green and terracotta live; 28 component sets and 9 standalone components on 30 documented pages; 66 Lucide icons; 25 text styles; a WCAG 2.2 AA pass with its measurements written on the components it changed; and 21 product screens with two click-through flows, built from the same components. Every variable, style and component carries a description of where it is used.
>
> The register is a warm paper notebook, not a SaaS dashboard: cream paper, warm brown text, forest green and terracotta. Documentation in English; the product's own copy stays in Spanish, as it ships.
...
**Lo que sigue abierto.** Los 108 rectángulos ocultos `Icon Left`/`Icon Right` de Button (pendiente A6). Rellenar el formulario y exportar las nueve imágenes. Y cuatro preguntas de diseño que el borrador de decisiones dejó a la vista y esta pasada no resuelve: `brand/primary-hover` aclara en claro y oscurece en oscuro; `color/field/placeholder` aliasa `text/secondary` mientras el producto pinta el placeholder con el terciario; los dos pasos con nombre de `Terracotta` rompen la convención numérica; y el set Button dibuja Md y Lg iguales a 36 px mientras el producto tiene 32 por defecto.
...
- **Terracota como texto** (etiquetas de marca favorita, `BrandStoreLink`, glifo de categoría): `--secondary` (`#C56A3E`) daba 3.55–3.70:1 en claro, bajo el 4.5:1 de texto normal → **resuelto el 28-ago-2026 con `--brand-secondary`** (`Terracotta/text` `#A44C1D`, 5.38–5.61:1), el mismo patrón relleno/primer-plano que `--brand`. En oscuro las dos props coinciden y ya cumplían. Ver «La terracota se parte como el verde».
