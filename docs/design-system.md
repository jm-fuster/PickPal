...
- [ ] **Los 22 estilos de texto no se aplican fuera de Foundations**: 0 de los 496 textos de las 29 páginas de componente usa un estilo; solo los 204 de `Foundations - Typography` e `Iconography`. El tipo sí está tokenizado a nivel de variable (`fontSize` vinculado en 168 de 170 textos de especímenes), así que no es texto crudo: es que se salta la capa de estilos. Decidir si los estilos se aplican en los especímenes o si son solo documentación de la escala — y escribirlo.
