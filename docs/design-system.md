...
- **Botón de marca favorita**: cuando la idea matchea una marca favorita (`matchFavoriteBrands`), antes de la rejilla de tiendas se renderiza un botón por marca a **ancho completo** (`col-span-2`), `buttonVariants({ size: "default", variant: "outline" })` tintado en `border-secondary/40 text-secondary hover:text-secondary` (mismo registro que el badge de marca), con icono `Tags` (`size-4`) + nombre de la marca (`<span className="truncate">`) + `ExternalLink` (`size-3.5`). Va **primero y a ancho completo** porque es la vía que de verdad funciona: muchas marcas favoritas son DTC (Brandy Melville y similares) que no se venden en los marketplaces, así que su enlace apunta a una búsqueda en Google acotada a la marca (`generateBrandSearchUrl` en `src/lib/brands.ts`), no a un marketplace. Los chips de tienda se mantienen debajo — algunas marcas (Nike, LEGO) sí están en Amazon y ahí la búsqueda de marketplace funciona. Sin logo de marca (eso sería trabajo futuro con un resolver marca→dominio); el icono `Tags` lo hermana con el badge. Solo en la card de generación (`GiftRecommendationCard`); la lista de "Ideas guardadas" todavía no muestra marcas.
