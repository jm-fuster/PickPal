...
- **Cabecera visual de las cards de ideas**: banda `h-24 rounded-xl` con dos variantes que comparten altura (las filas mixtas quedan alineadas):
  - **Foto de stock (Pexels)** si la generación encontró una: `<img>` con `w-full object-cover rounded-xl bg-muted/40`, `alt="" aria-hidden` (ilustrativa, el título ya nombra el regalo), `loading="lazy"`. Si la carga falla (`onError`), la card cae en caliente a la variante de icono.
