...
**Lo que queda, y es decisión de diseño, no trabajo mecánico**: 106 textos de especímenes cuyo estilo les movería entre 1 y 5 px por línea. El grupo que decide es **58 etiquetas de Geist Medium 14 con interlineado 20px, que `Label 2` aprieta a 16,8 — 3,2px menos por línea**, seguido de 3 a −4,8px y 10 a +4px. Ahí hay que elegir de quién es la verdad: de la escala (ratios documentados, estilo comunidad) o de los especímenes (píxeles puestos a mano). Y **los 28 sin hueco piden 3–4 estilos nuevos o normalizar los nodos**: `Geist Medium 12` con tracking 0 (9 nodos, `Label 3` pide 1 %), `Geist Medium 11` con 12 % sin mayúsculas (6), `Geist Medium 18` (6, la escala Label acaba en 16), `Geist Light 24` (2, el peso Light no está en la escala), `Geist Medium 13` (2) y `Geist Regular 24` (1, Body acaba en 20).
...
**Lo que queda abierto**: dos entradas de Excepciones citan en presente `color/chart/3` y `color/chart/5` —**no existe ni una variable con «chart» en el nombre**— y `color/icon/category-amber`, cuando la familia de categoría es solo `color/bg/category/*` (5 miembros, sin gemelo de icono). O se borraron y la entrada debe decirlo, o falta crearlos: es decisión, no corrección. Dos frases históricas conservan nombres muertos a propósito (`size/interactive-track`, `size/interactive-2xs`), porque rehacerlas obligaría a reconstruir toda la cadena de renombrados. Y las cifras de Iconography sobre el código —«87 renders, 82 importados y 5 dinámicos», «68 identificadores → 66 glifos»— no se han recontado: piden el mismo método que las produjo.
...
- [ ] **Estilos de texto, ola 2 — de quién es la verdad**: quedan 106 textos de especímenes cuyo estilo les movería 1–5 px por línea. El grupo que decide son **58 etiquetas Geist Medium 14 con interlineado 20px** que `Label 2` aprieta a 16,8 (−3,2px), más 3 a −4,8px y 10 a +4px. O manda la escala (ratios) o mandan los especímenes (píxeles a mano); mientras no se decida, esos 106 siguen escritos a mano. Ver «Estilos de texto».
- [ ] **28 textos de especímenes sin hueco en la escala**: `Geist Medium 12` con tracking 0 (9), `Geist Medium 11` con 12 % sin mayúsculas (6), `Geist Medium 18` (6), `Geist Light 24` (2), `Geist Medium 13` (2), `Geist Regular 24` (1), más las 3 iniciales del Avatar, que son Regular y por rol pedirían un `Label`. Decidir si nacen 3–4 estilos o se normalizan los nodos.
- [ ] **`TextColumn` de Upcoming Date Card recorta el texto**: la columna mide 93px (FILL, `clipsContent`) y sus tres textos son HUG de 109, 117 y 129px con `textTruncation: DISABLED`, así que se cortan a media palabra — el nombre incluido. La card es 340 FIXED y entre el avatar (48) y el Button (143, HUG) no queda ancho. Defecto anterior a la ola 1 de estilos, detectado al capturarla. Arreglo: los textos a FILL —y decidir si el Button deja de ser HUG—, la misma trampa que ya está anotada para la página de Excepciones.
...
- [ ] **`Button Icon` en `Size=Icon-Sm` lleva un icono de 16, no de 14**: los 6 variantes Sm muestran el icono al mismo tamaño que los Icon normales, mientras todo botón `Size=Sm` usa `sizing/control/icon-sm` (14). Decidir si es intencionado; si no, bajar los 6 a 14 y repuntar el binding.
- [ ] **`color/chart/*` y `color/icon/category-amber` se citan y no existen**: dos entradas de `Foundations - Excepciones` los nombran en presente (la de rampas 50–950 y la de categorías). No hay ninguna variable con «chart» en el nombre, y de categoría solo existe `color/bg/category/*` (5, sin gemelo de icono). Decidir si se borraron —y entonces la entrada debe decirlo— o si faltan por crear.
