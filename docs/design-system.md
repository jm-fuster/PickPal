...
| `color/icon/category-amber` | `--category-amber` | Glifo de las ~15 categorías de regalo ámbar de `giftImages.ts`, sobre `bg-chart-3/15`. **Decorativo, no un aviso**. **Ya no existe en Figma** (28-ago-2026): se perdió al reducir `color/` a grupos de rol, mientras la custom property sigue viva y en uso. Es el único color del producto sin token que lo nombre  |
