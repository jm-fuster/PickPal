...
- **Tienda oficial resuelta (Brandfetch)**: al generar, `/api/recommendations` resuelve cada marca matcheada a su tienda con una llamada a Brandfetch Search (`GET /v2/search/{marca}?c={clientId}`), que devuelve dominio + logo en una sola petición. Se adjunta a la idea como `matchedBrandStores` (igual que la foto de Pexels: enriquecimiento server-side tras la generación, validado por prefijo antes de persistir). La card muestra el **logo de la marca** y enlaza a `"{producto} site:{dominio}"` en Google (`generateBrandStoreSearchUrl`), que aterriza en las páginas de producto de esa tienda sin depender de su buscador propio.
